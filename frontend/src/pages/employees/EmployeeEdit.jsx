...
export default function EmployeeEdit() {
  ...
  const [formData, setFormData] = useState({
    ...
    salary_type: '', basic_salary: '', joining_date: '', is_active: true,
  ...
  const fetchEmployeeData = async () => {
    try {
      const response = await api.get(`/api/employees/${id}/edit`);
      const empData = response.data.employee;
      const empProfile = empData.employee || {};
      setBranches(response.data.branches);
      setDepartments(response.data.departments);
      setDesignations(response.data.designations);
      setFormData({
        name: empData.name || '', email: empData.email || '', password: '',
        phone: empProfile.phone || '', dob: empProfile.dob ? empProfile.dob.split('T')[0] : '',
        gender: empProfile.gender || 'male', address: empProfile.address || '',
        branch_id: empProfile.branch_id || '', department_id: empProfile.department_id || '',
        designation_id: empProfile.designation_id || '', salary_type: empProfile.salary_type || 'Monthly',
        basic_salary: empProfile.basic_salary || '0',
        joining_date: empProfile.joining_date ? empProfile.joining_date.split('T')[0] : '',
        is_active: empData.is_active === 1,
        account_holder_name: empProfile.account_holder_name || '', account_number: empProfile.account_number || '',
        bank_name: empProfile.bank_name || '', bank_branch: empProfile.bank_branch || '',
        ifsc_code: empProfile.ifsc_code || '', pan_number: empProfile.pan_number || ''
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post(`/api/employees/${id}`, submitData, { headers: { 'Content-Type': 'multipart/form-data' } });
      navigate('/employees');
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200/80 p-6 sm:p-8">
          ...
          <div className="space-y-7">
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className={labelCls}>Password <span className="text-gray-400 font-normal normal-case text-[10px]">(Leave blank to keep)</span></label><input type="password" name="password" value={formData.password} onChange={handleInputChange} className={inputCls} placeholder="••••••••" /></div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className={labelCls}>Salary Type</label><select name="salary_type" value={formData.salary_type} onChange={handleInputChange} className={inputCls}><option value="Monthly">Monthly</option><option value="Weekly">Weekly</option><option value="Hourly">Hourly</option></select></div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" name="basic_salary" value={formData.basic_salary} onChange={handleInputChange} className={`${inputCls} pl-7`} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-7">
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className={labelCls}>Bank Branch</label><input type="text" name="bank_branch" value={formData.bank_branch} onChange={handleInputChange} className={inputCls} /></div>
                <div><label className={labelCls}>PAN Number</label><input type="text" name="pan_number" value={formData.pan_number} onChange={handleInputChange} className={`${inputCls} font-mono uppercase`} /></div>
              </div>
            </div>
            <div>
              <h2 className="text-base font-bold text-gray-800 border-b border-gray-100 pb-3 mb-5">Update Documents <span className="text-xs font-normal text-gray-400 normal-case ml-1">(Leave blank to keep existing)</span></h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {step === 3 && (
          <div className="space-y-5">
            <h2 className="text-base font-bold text-gray-800 border-b border-gray-100 pb-3 mb-5">Review Updates</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="p-5 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-y-2.5 text-sm">
                  ...
                  <div className="text-gray-400">Status:</div><div className={formData.is_active ? 'text-[#62975f] font-bold' : 'text-red-500 font-bold'}>{formData.is_active ? 'Active' : 'Inactive'}</div>
                </div>
              </div>
              <div className="p-5 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-y-2.5 text-sm">
                  ...
                  <div className="text-gray-400">Salary:</div><div className="text-gray-800 font-medium">₹{formData.basic_salary}</div>
                </div>
              </div>
            </div>
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex gap-2.5 mt-4">
              ...
              <p className="text-sm text-amber-700 font-medium">You are about to save changes. Please verify everything is correct.</p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-8 pt-5 border-t border-gray-100">
          ...
          {step < 3 ? (
            ...
            <button type="submit" disabled={saving} className="px-6 py-2.5 bg-[#299dc6] hover:bg-[#2389ae] text-white rounded-lg font-semibold text-sm shadow-sm transition-colors disabled:opacity-50 flex items-center gap-2 cursor-pointer">
              {saving ? <><svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Updating...</> : 'Update Employee'}
            </button>
          )}
        </div>
      </form>
